...
@synthesize percentComplete, operationIsOK, appendIfExist, possibleFilename, completion, operationFinished, operationBreaked, dataInfo, type;
...
//    [request addValue: [NSString stringWithFormat: @"bytes=%.0lu-", (unsigned long)data.length] forHTTPHeaderField: @"Range"];
    ...
    NSFileHandle *fileHandle = [NSFileHandle fileHandleForReadingAtPath:[NSString stringWithFormat:@"%@.%@", [self filePath], type]];
...
//    [Records addValue:[self downloadData] andvId:[dataInfo[@"infor"] getValueFromKey:@"id"] andKey:dataInfo[@"name"]];
    ...
    [[NSFileManager defaultManager] createFileAtPath:[NSString stringWithFormat:@"%@.%@", [self filePath], type] contents:nil attributes:nil];
...
{
    NSFileHandle *hFile = [NSFileHandle fileHandleForWritingAtPath:[NSString stringWithFormat:@"%@.%@", [self filePath], type]];
    ...
    @catch (NSException * e)
    {
        NSLog(@"exception when writing to file %@", [NSString stringWithFormat:@"%@.%@", [self filePath], type]);
    }
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    {
        [[NSFileManager defaultManager] removeItemAtPath:[NSString stringWithFormat:@"%@.%@", [self filePath], type] error:NULL];
